...
export async function getCookies(options: GetCookiesOptions): Promise<GetCookiesResult> {
	...
	for (const browser of browsers) {
		...
		if (browser === 'chrome') {
			...
			if (edgeProfile) edgeOptions.profile = edgeProfile;
			if (options.timeoutMs !== undefined) edgeOptions.timeoutMs = options.timeoutMs;
			if (options.includeExpired !== undefined) edgeOptions.includeExpired = options.includeExpired;
			if (options.debug !== undefined) edgeOptions.debug = options.debug;
